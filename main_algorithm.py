...
bases_de_dados = [base_1]
nomes_das_bases = ['base_1']

doc2vec_model_file = Doc2Vec.load("doc2vec_models/meu_modelo_doc2vec.pt")

# lista_algoritmos = ['regLog', 'svm', 'extraTrees', 'randomForest', 'naiveBayes', 'random_predictions']
lista_algoritmos = ['svm']
#pre_processamentos = ['limpo', 'removeStopwords', 'lem', 'stem', 'doc2vec']
pre_processamentos = ['removeStopwords']
...
for i, base in enumerate(bases_de_dados):
    ...
    print("Nome da base: ", nomes_das_bases[i])
    if nomes_das_bases[i].endswith('1'):
        base_index = '1'
    elif nomes_das_bases[i].endswith('2'):
        base_index = '2'
    elif nomes_das_bases[i].endswith('3'):
        base_index = '3'
    else: 
        base_index = str(i)
    ...
    for item in pre_processamentos:
        for algoritmo in lista_algoritmos:
            ...
            if alg == 'random_predictions':
                print("Realizando previsões aleatórias")
                random_results = random_prediction(classes)
                print("RESULTADO DO TESTE ALEATORIO: ", random_results)
                scores_title = 'score_' + 'base' + base_index + '_random_predictions_' + '.pkl'
                destino = os.path.join(pasta_resultados, scores_title)
                export_to_pkl(random_results, destino)
                ...
                if(item=='limpo'):
                    ...
                    scores_limpo = cross_validate(pipeline, comentarios, classes, cv=cv_number, scoring=metricas)
                    ...
                    scores = cross_validate(pipeline, comentarios_formatados, classes, cv=cv_number, scoring=metricas)
                    scores_title = 'score_' + 'base' + base_index + '_' + algoritmo + '_' + item + '.pkl'
                    destino = os.path.join(pasta_resultados, scores_title)
                    ...
                    scores_limpo = cross_validate(pipeline, comentarios_formatados, classes, cv=cv_number, scoring=metricas)
                    scores_title = 'score_' + 'base' + base_index + '_' + algoritmo + '_' + item + '.pkl'
                    destino = os.path.join(pasta_resultados, scores_title)
                    export_to_pkl(scores_limpo, destino)   
